...
function aghsfort_lion_finger_of_death:GetCooldown(iLevel)
	if IsValid(self.shard_charge) then
		return 0
	end
...
function modifier_aghsfort_lion_finger_of_death:updateData(kv)
	-- print("update")
	local damage_layer = self.ability:GetSpecialValueFor("damage_per_layer")
	local hp_layer = self.ability:GetSpecialValueFor("hp_per_layer")
...
function modifier_aghsfort_lion_finger_of_death_debuff:updateData(kv)
	if IsServer() then
		self.layer = 1
		if IsAghanimConsideredHero(self.parent) and not IsIgnoredIllusion(self.parent) then
