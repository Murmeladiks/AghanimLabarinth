...
function aghsfort_lion_impale:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if IsValid(hTarget) and IsValid(caster) then
		...
		if ExtraData.thinker_id then
			...
			-- if hit_table then
			-- 	if hit_table[target_id] then
			-- 		-- print("hitted!")
			-- 		return
			-- 	end
			-- 	hit_table[target_id] = true
			-- 	bSecondary = 1
			-- end
			-- thinker version
			-- if self._last_target == target_id and self._last_thinker == ExtraData.thinker_id then
			-- 	return
			-- end
			-- local thinker = EntIndexToHScript(ExtraData.thinker_id)
			-- if IsValid(thinker) and thinker._hit_table then
			-- 	if TableContainsValue(thinker._hit_table, target_id) then
			-- 		-- print("hitted!")
			-- 		return
			-- 	end
			-- 	table.insert(thinker._hit_table, target_id)
			-- 	-- thinker._hit_table[target_id] = true
			-- 	self._last_target = target_id
			-- 	self._last_thinker = ExtraData.thinker_id
			-- 	bSecondary = 1
			-- else
			-- 	return
			-- end
